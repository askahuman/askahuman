...
## Context
`platia` (our Astro reference) uses SSR with `@astrojs/node` because it has server endpoints
...
## Decision
- Mirror platia's stack (Astro 5 + React 19 islands + Tailwind 4 + bun) but set
  `output: 'static'` — `astro build` emits a pure static `dist/`.
- Add what platia lacks: `vite-plugin-pwa` (service worker + Web App Manifest), client crypto
...
- Design tokens (fonts JetBrains Mono + IBM Plex Sans, accent approve `#39d98a` /
  ...
  `frontend/initial-design/` and platia's `global.css` conventions.
...
- React is used only as Astro islands for the interactive card/pairing screens; the shell is
  static HTML/CSS.
- Divergence from platia (SSR) is justified: no server-side work exists to do.
