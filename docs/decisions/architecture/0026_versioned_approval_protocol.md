...
The same device identity authenticates subscription changes: a push-subscription signature binds v2, room, a monotonically increasing push sequence, endpoint, and both subscription keys. This preserves the existing subscription value/API semantics while preventing a copied session key from installing a different push endpoint. Push transport restrictions remain independently enforced.

## Recorded-message rollback and persistent counters

A new request receives one positive, safe-integer `request_seq` from the paired agent session; every re-announcement retains it. The phone records the largest verified sequence and its request digest in a per-pair IndexedDB ledger before displaying a card. Older sequences and equal sequences with different digests cannot replace the current card, including when an older authentic request was never previously delivered. Equal identical requests remain valid re-announcements without a new deadline. Decisions and receipts bind the sequence through the original request digest. Exact retained decision/receipt retries remain valid even when newer requests have advanced the display sequence.
