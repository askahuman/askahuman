...
//
// Spawns the real `agent ask` (role A), reads the PAIR line it prints, drives the
// REAL PWA in headless Chromium (role B) to approve the request, and asserts the
// agent receives {approved:true}. Nothing is mocked: relay + web run in kind, the
// agent is the real binary, the browser runs the real bundle.
//
...
// 1) Spawn the agent (role A). It prints `PAIR payload=... code=... room=...` to
//    stderr, pairs, sends one request, then prints the decision JSON to stdout.
const args = ['ask', '--relay', RELAY_WS, '--web', WEB_ORIGIN, '--print-pair',
...
const payload = await new Promise((resolve, reject) => {
  const to = setTimeout(() => reject(new Error('timeout waiting for PAIR line')), 15000);
  agent.stderr.on('data', (d) => {
    stderr += d;
    const m = stderr.match(/PAIR\s+payload=(\S+)/);
    if (m) { clearTimeout(to); resolve(m[1]); }
  });
  agent.on('exit', (c) => { clearTimeout(to); reject(new Error(`agent exited early (${c})\n${stderr}`)); });
}).catch(fail);

log(`paired payload captured (${payload.slice(0, 24)}…)`);

// 2) Drive the real PWA as the phone (role B). ignoreHTTPSErrors mimics a phone
//    that has the mkcert root CA installed (trusts the local TLS proxy).
const browser = await chromium.launch();
const context = await browser.newContext({ ignoreHTTPSErrors: true });
const page = await context.newPage();
page.on('console', (m) => { if (m.type() === 'error') console.error('  [pwa console.error]', m.text()); });
const url = `${WEB_ORIGIN}/app#p=${payload}`;
log(`pwa: goto ${url}`);
await page.goto(url, { waitUntil: 'domcontentloaded' });
const sec = await page.evaluate(() => ({ secure: window.isSecureContext, cam: !!(navigator.mediaDevices && navigator.mediaDevices.getUserMedia) }));
log(`pwa: secure context=${sec.secure} · in-app camera API available=${sec.cam}`);
...
log(`\nPWA LIVE E2E PASSED (kind=${KIND}) — real browser approved through the kind relay; screenshot: ${SHOT}`);
process.exit(0);
