...
describe("decodeRequest", () => {
  ...
  it("rejects a decision masquerading as a request", () => {
    expect(() => decodeRequest(enc({ kind: "decision", id: "x" }))).toThrow(
      /request metadata/,
    );
  });
  it("rejects a request missing id", () => {
    expect(() =>
      decodeRequest(
        enc({ kind: "request", title: "T", response: { kind: "yesno" } }),
      ),
    ).toThrow(/id/);
  });
...
describe("decision builders", () => {
  it("builds yesno/choice/text decisions matching pkg/wire shape", () => {
    expect(
      JSON.parse(
        new TextDecoder().decode(encodeDecision(newYesNoDecision("id1", true))),
      ),
    ).toEqual({
      kind: "decision",
      id: "id1",
    ...
    expect(newChoiceDecision("id2", "Proceed").result).toEqual({
      choice: "Proceed",
    });
...
describe("encodeDecision padding (privacy: no length leak)", () => {
  ...
  it("padded plaintext still decodes (trailing whitespace ignored)", () => {
    const d = decodeDecision(
      encodeDecision({
        ...newYesNoDecision("req_8f3a", true),
        protocol: 2,
        room: "0123456789abcdef",
...
describe("decodeDecision validation", () => {
  ...
  it("rejects a request masquerading as a decision", () => {
    expect(() => decodeDecision(enc({ kind: "request", id: "x" }))).toThrow(
      /decision metadata/,
    );
  });
  it("rejects a missing id and an over-long id", () => {
    expect(() => decodeDecision(enc({ kind: "decision", result: {} }))).toThrow(
      /id/,
    );
    expect(() =>
      decodeDecision(
        enc({ kind: "decision", id: "a".repeat(300), result: {} }),
      ),
    ).toThrow(/id/);
  });
  it("rejects a non-boolean approved", () => {
    expect(() =>
      decodeDecision(
        enc({ kind: "decision", id: "a", result: { approved: "yes" } }),
      ),
    ).toThrow(/boolean/);
  });
});
...
describe("decodeRequest bounded validation", () => {
  ...
  it("rejects too many options", () => {
    expect(() =>
      decodeRequest(
        enc({
          ...base,
          response: { kind: "choice", options: Array(40).fill("x") },
        }),
      ),
    ).toThrow(/choice/);
  });
  it("rejects out-of-bounds expires_in_s", () => {
    expect(() =>
      decodeRequest(
        enc({ ...base, response: { kind: "yesno" }, expires_in_s: 999999 }),
      ),
    ).toThrow(/integer/);
  });
});
...
describe("decisionSigningMessage (cross-language byte contract)", () => {
  ...
  it.each([
    [
      "yesno true",
      { approved: true },
      "6161683a6465636973696f6e3a76310030313233343536373839616263646566007265715f31007965736e6f3a31",
    ],
    [
      "yesno false",
      { approved: false },
      "6161683a6465636973696f6e3a76310030313233343536373839616263646566007265715f31007965736e6f3a30",
    ],
    [
      "choice",
      { choice: "Merge & retry" },
      "6161683a6465636973696f6e3a76310030313233343536373839616263646566007265715f310063686f6963653a4d657267652026207265747279",
    ],
    [
      "text",
      { text: "up to $500" },
      "6161683a6465636973696f6e3a76310030313233343536373839616263646566007265715f3100746578743a757020746f2024353030",
    ],
  ] as const)(
    "pins the canonical message bytes for %s",
    (_name, result, wantHex) => {
...
describe("device_key wire message", () => {
  ...
  it("rejects a non-device_key kind and a missing public_key", () => {
    expect(() =>
      decodeDeviceKey(enc({ kind: "vapid_key", public_key: "x" })),
    ).toThrow(/not a device_key/);
    expect(() => decodeDeviceKey(enc({ kind: "device_key" }))).toThrow(
      /public_key missing/,
    );
  });
});
...
describe("decodeDecision sig tolerance + cap", () => {
  ...
  it("accepts a decision with a sig and without one", () => {
    ...
    const without = decodeDecision(
      enc({ kind: "decision", id: "a", result: { approved: true } }),
    );
