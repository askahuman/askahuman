...
# it tees the pairing CODE to a log file so the test harness can read the code and TYPE it
# into the real PWA. stdout/stdin stay pristine for MCP JSON-RPC; only stderr is teed.
set -euo pipefail
...
exec ./bin/agent serve \
  --relay "${AAH_RELAY_URL:-ws://127.0.0.1:8080/ws}" 2>>"$LOG"
